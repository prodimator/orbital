...
test('surface proximity rewards close passes with a bounded nonlinear bonus',()=>{
  ...
  assert.equal(rate(100),0);assert.equal(rate(0),20);
  assert.equal(rate(50),5);assert.ok(rate(10)>rate(30));
...
test('total combines time and flying points and freezes on death',()=>{
  const s=createRun(()=>.3);step(s,{});
  assert.ok(s.flyingPoints>0);assert.equal(s.score,s.time+s.flyingPoints);
  s.alive=false;const score=s.score;step(s,{},1);assert.equal(s.score,score);
  assert.equal(createRun().score,0);
});
test('scoring is identical across frame rates',()=>{
  const a=createRun(()=>.3),b=createRun(()=>.3);
