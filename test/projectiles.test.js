...
test('shots require flight and respect cooldown', () => {
  ...
  assert.equal(s.projectiles.length, 1);
  assert.ok(
    s.projectiles[0].vx * Math.cos(s.ship.angle) + s.projectiles[0].vy * Math.sin(s.ship.angle) >
      300,
  );
});
test('projectile crossing destroys asteroid and awards size-based points once', () => {
  ...
  updateProjectiles(s, {}, 0.1, [], sweptHit);
  assert.equal(s.asteroids.length, 0);
  assert.equal(s.projectiles.length, 0);
  assert.equal(s.shootingPoints, asteroidReward(8));
  updateProjectiles(s, {}, 0.1, [], sweptHit);
  assert.equal(s.shootingPoints, asteroidReward(8));
