...
export function resolveCompact({coarse, hoverless, override}) {
  return override === null || override === undefined ? coarse && hoverless : override;
}
...
export function watchCompact(override, onChange) {
  ...
  const apply = () =>
    onChange(resolveCompact({coarse: coarse.matches, hoverless: hoverless.matches, override}));
...
export function watchTouch(onChange) {
  ...
  addEventListener(
    'mousemove',
    () => {
      if (performance.now() - lastTouch > 1000) onChange(false);
    },
    {capture: true},
  );
}
