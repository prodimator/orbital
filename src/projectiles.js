...
export function updateProjectiles(s, input, dt, oldBodies, sweptHit) {
  s.weaponHeat = Math.max(
    0,
    s.weaponHeat - (s.overheated ? OVERHEAT_COOLING_RATE : COOLING_RATE) * dt,
  );
  if (s.overheated && s.weaponHeat < 1e-8) {
    s.weaponHeat = 0;
    s.overheated = false;
  }
  ...
  s.projectiles = s.projectiles.filter((p) => {
    ...
    for (let i = 0; i < s.bodies.length; i++) {
      const b = s.bodies[i],
        before = oldBodies[i];
      const t = hitTime(
        {x: old.x - before.x, y: old.y - before.y},
        {x: p.x - b.x, y: p.y - b.y},
        p.r + b.r,
      );
      if (t < first) {
    ...
    for (const a of s.asteroids) {
      const t = hitTime(
        {x: old.x - (a.oldX ?? a.x), y: old.y - (a.oldY ?? a.y)},
        {x: p.x - a.x, y: p.y - a.y},
        p.r + a.r,
      );
      if (t < first) {
    ...
    if (first !== Infinity) {
      if (target) {
        s.asteroids = s.asteroids.filter((a) => a !== target);
        if (s.alive) s.shootingPoints += asteroidReward(target.r);
      }
      return false;
    }
