...
function frame(now){const elapsed=(now-last)/1000;last=now;if(mode==='playing'){advance(state,input,elapsed);if(!state.alive)finish();}$('heat-fill').style.width=state.weaponHeat+'%';$('heat-value').textContent=Math.round(state.weaponHeat)+'%';$('heat-meter').setAttribute('aria-valuenow',Math.round(state.weaponHeat));$('weapon').classList.toggle('overheated',state.overheated);$('weapon-label').textContent=state.overheated?'OVERHEATED · COOLING':'LASER ENERGY USED';$('score').textContent=state.score.toFixed(1);$('shooting').textContent=state.shootingPoints.toFixed(0);$('flying').textContent=state.flyingPoints.toFixed(1);$('survival').textContent=state.time.toFixed(1);$('bonus').textContent=(mode==='playing'&&state.phase==='flying'?proximityRate(state.ship,state.bodies):0).toFixed(1);const time=format(state.time);$('time').textContent=time.slice(0,-3);$('fraction').textContent=time.slice(-3);$('speed').textContent=Math.hypot(state.ship.vx,state.ship.vy).toFixed(1);const g=gravity(state.ship,state.bodies);$('gravity').textContent=Math.hypot(g.x,g.y).toFixed(1);$('flight-status').textContent=mode==='playing'?(state.phase==='landed'?'↑ PRESS UP TO LAUNCH':input.up?'● THRUST ACTIVE':'● COASTING'):mode==='paused'?'Ⅱ FLIGHT PAUSED':mode==='ended'?'○ SIGNAL LOST':'● SYSTEM READY';view.draw(state,input.up,mode==='playing',elapsed);requestAnimationFrame(frame);}
