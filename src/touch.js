...
export function stickVector(origin, point, radius) {
  ...
  return {
    ...
    deflection,
    x: (dx / distance) * deflection,
    y: (dy / distance) * deflection,
  };
}
...
export function aimInput(stick, current) {
  return stick && stick.deflection >= MIN_DEFLECTION
    ? steerToward(current, stick.angle)
...
export function createTouchControls({input}) {
  ...
  function release() {
    drop();
    for (const reset of buttons) reset();
  }
  return {
    aim: () => vector,
    release,
