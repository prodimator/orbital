...
function orbit(b, t) {
  ...
  b.y = Math.sin(a) * b.orbit;
}
export function createRun(random = Math.random) {
  const bodies = [
    {name: 'HELIOS', x: 0, y: 0, r: 40, mu: 180000 * GRAVITY_STRENGTH, color: 0xffc674},
  ];
  const count = 2 + Math.floor(random() * 3);
  for (let i = 0; i < count; i++) {
    const r = 15 + random() * 30,
      distance = 175 + i * 145;
    const b = {
      name: ['VERDANT', 'VESPER', 'EMBER', 'BOREAL'][i],
      r,
      mu: r * r * 15 * GRAVITY_STRENGTH * PLANET_GRAVITY_MULTIPLIER,
      color: colors[i],
      orbit: distance,
      phase: random() * Math.PI * 2,
      omega: Math.sqrt(180000 / distance ** 3),
    };
    orbit(b, 0);
    bodies.push(b);
  }
  ...
  const ship = {
    x: home.x + Math.cos(angle) * (home.r + 3.5),
    y: home.y + Math.sin(angle) * (home.r + 3.5),
    vx: -home.y * home.omega,
    vy: home.x * home.omega,
    angle,
    r: 3.5,
  };
  return {
    ...
    asteroids: [],
    asteroidId: 0,
    asteroidSeed: Math.floor(random() * 4294967296),
    nextAsteroid: 6,
    bodies,
    ship,
    home: home.name,
...
export function step(s, input, dt = DT) {
  ...
  if (s.phase === 'landed') {
    ...
    const home = s.bodies.find((b) => b.name === s.home),
      p = s.ship;
    const angle = Math.atan2(home.y, home.x),
      distance = home.r + p.r;
    p.angle = angle;
    p.x = home.x + Math.cos(angle) * distance;
    p.y = home.y + Math.sin(angle) * distance;
    p.vx = -p.y * home.omega;
    p.vy = p.x * home.omega;
    if (input.up) {
      s.phase = 'flying';
      p.x += Math.cos(angle) * 0.1;
      p.y += Math.sin(angle) * 0.1;
  ...
  const p = s.ship,
    old = {x: p.x, y: p.y},
    previous = s.bodies.map((b) => ({x: b.x, y: b.y}));
  ...
  s.bodies.forEach((b, i) => {
    ...
    if (
      sweptHit(
        {x: old.x - previous[i].x, y: old.y - previous[i].y},
        {x: p.x - b.x, y: p.y - b.y},
        p.r + b.r,
      )
    ) {
      s.alive = false;
      s.reason = `Impact with ${b.name}`;
    }
  });
  if (Math.hypot(p.x, p.y) > LIMIT) {
    s.alive = false;
    s.reason = 'Lost beyond the flight perimeter';
  }
...
export function advance(s, input, elapsed) {
  s.accumulator += Math.min(elapsed, 0.1);
